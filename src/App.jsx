...
import { fetchNotices, isOnline, storage, API_BASE, registerUserSiteOnServer, removeUserSiteFromServer, deleteNoticeFromServer } from './api';
...
export default function App() {
  ...
  const deleteAutoNotice = async (id) => {
    if (!confirm('이 공고를 삭제할까요? 목록에서 완전히 제거됩니다.')) return;
    const next = serverNotices.filter(n => n.id !== id);
    setServerNotices(next);
    await storage.setCachedNotices(next);
    const result = await deleteNoticeFromServer(id);
    if (!result.ok) {
      alert('서버에서 삭제하지 못했어요. 인터넷 연결을 확인해주세요. (화면에서는 일단 숨겨졌어요)');
    }
  };

  const deleteNotice = (id) => {
    if (id && id.startsWith('m_')) return deleteManualNotice(id);
    return deleteAutoNotice(id);
  };
        ...
        <main className="px-5 py-5">
          ...
          {tab === 'notices' && (
            <NoticesView notices={filteredNotices} search={search} setSearch={setSearch}
              ...
              onDelete={deleteNotice} onOpenUrl={openUrl}
